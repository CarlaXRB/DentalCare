...
@section('title', __('Ver Radiografía'))
@section('subtitle')
{{ __('Ver Radiografía') }}
@endsection
@section('content')
...
<div class="max-w-5xl pt-2 mx-auto bg-white rounded-xl p-8 text-gray-900 dark:text-white">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 pb-5 text-black dark:text-white">
        <div class="flex gap-2">
            ...
            <a href="{{ route('patient.show', $radiography->patient->id ) }}" class="txt text-green-500 hover:text-green-700 hover:font-bold pl-12">{{ __('Ver Paciente') }}</a>
            @else
            <p class="text-red-500 mb-3">{{ __('Paciente no registrado en la base de datos.') }}</p>
            @endif
        </div>
        <div class="flex gap-2">
            <h3 class="title4">{{ __('ID de la radiografía:') }}</h3><span class="txt">{{ $radiography->radiography_id }}</span>
        </div>
        <div class="flex gap-2">
            <h3 class="title4">{{ __('Fecha del estudio:') }}</h3><span class="txt">{{ $radiography->radiography_date }}</span>
        </div>
        <div class="flex gap-2">
            <h3 class="title4">{{ __('Tipo de radiografía:') }}</h3><span class="txt">{{ $radiography->radiography_type }}</span>
        </div>
        <div class="flex gap-2">
    ...
    <div class="flex justify-center">
        <a href="{{ route('radiography.tool', $radiography->id) }}" class="botton2">{{ __('Herramientas') }}</a>
    </div>

    <!-- Acciones Editar / Eliminar -->
    <div class="flex justify-end pl-3">
        <a href="{{ route('radiography.edit', $radiography->id ) }}" class="botton3">{{ __('Editar') }}</a>
        @auth
