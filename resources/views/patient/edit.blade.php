...
<div class="bg-white rounded-lg max-w-5xl mx-auto">
    <form method="POST" action="{{ route('patient.update', $patient->id) }}">
        @method('PUT')
        @csrf
        <h1 class="title1 text-center mb-8">{{ __('Edit Patient Information') }}</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="gender" 
                    ...
                    <option value="Femenino" {{ $patient->gender == 'Femenino' ? 'selected' : '' }}>{{ __('Female') }}</option>
                    <option value="Masculino" {{ $patient->gender == 'Masculino' ? 'selected' : '' }}>{{ __('Male') }}</option>
                </select>
            ...
            <div>
                ...
                <input type="text" name="family_contact" value="{{ $patient->family_contact }}" 
                    class="border-gray-300 rounded-lg p-3 w-full focus:outline-none focus:ring-2 focus:ring-cyan-500"/>
